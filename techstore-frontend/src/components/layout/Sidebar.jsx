import { NavLink, useNavigate } from 'react-router-dom';
import { Truck, ShoppingCart, Tag, Monitor, LogOut } from 'lucide-react'; // 1. Adicionado o ícone LogOut
import { useAuth } from '../../contexts/AuthContext'; // 2. Importado o seu contexto de autenticação

const menuItems = [
  { to: '/produtos', label: 'Produtos', icon: ShoppingCart, enabled: true },
  { to: '/categorias', label: 'Categorias', icon: Tag, enabled: true },
  { to: '/fornecedores', label: 'Fornecedores', icon: Truck, enabled: true },
];

function Sidebar() {
  const { logout } = useAuth(); // 3. Puxando a função de logout do contexto
  const navigate = useNavigate();

  const handleLogout = () => {
    logout(); // Limpa o estado global e o localStorage
    navigate('/login'); // Redireciona o usuário para a tela de login
  };

  return (
    <aside className="w-64 bg-gray-900 text-white flex flex-col flex-shrink-0">
      {/* Logo / Título */}
      <div className="px-6 py-5 border-b border-gray-700">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-blue-600 rounded-lg flex items-center justify-center">
            <Monitor className="w-5 h-5 text-white" />
          </div>
          <div>
            <h1 className="text-lg font-bold leading-tight">TechStore</h1>
            <p className="text-xs text-gray-400">Gerenciador de Estoque</p>
          </div>
        </div>
      </div>

      {/* Menu de Navegação */}
      <nav className="flex-1 px-3 py-4">
        <p className="px-3 mb-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">
          Menu
        </p>
        <ul className="space-y-1">
          {menuItems.map((item) => (
            <li key={item.to}>
              {item.enabled ? (
                <NavLink
                  to={item.to}
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                      isActive
                        ? 'bg-blue-600 text-white'
                        : 'text-gray-300 hover:bg-gray-800 hover:text-white'
                    }`
                  }
                >
                  <item.icon className="w-5 h-5" />
                  {item.label}
                </NavLink>
              ) : (
                <span className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-gray-600 cursor-not-allowed">
                  <item.icon className="w-5 h-5" />
                  {item.label}
                  <span className="ml-auto text-[10px] bg-gray-700 text-gray-400 px-1.5 py-0.5 rounded">
                    Em breve
                  </span>
                </span>
              )}
            </li>
          ))}
        </ul>
      </nav>

      {/* Rodapé modificado para incluir o Botão de Sair */}
      <div className="p-3 border-t border-gray-700 flex flex-col gap-2">
        <button
          onClick={handleLogout}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-red-400 hover:bg-red-950/40 hover:text-red-300 transition-colors"
        >
          <LogOut className="w-5 h-5" />
          Sair do Sistema
        </button>
        
        <div className="px-3 pt-2 border-t border-gray-800/60">
          <p className="text-[11px] text-gray-500">TechStore Info</p>
          <p className="text-[11px] text-gray-600">TechStore Company</p>
        </div>
      </div>
    </aside>
  );
}

export default Sidebar;