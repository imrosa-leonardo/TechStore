                ...
                <table className="w-full">
                    <thead>
                        <tr className="bg-gray-50 border-b border-gray-200">
                            ...
                            <th className="text-right text-xs font-semibold text-gray-500 uppercase tracking-wider px-6 py-3">Preço</th>
                            <th className="text-center text-xs font-semibold text-gray-500 uppercase tracking-wider px-6 py-3">Estoque</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {produtosFiltrados.map((produto) => (
                            <tr key={produto.id} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    {produto.categoria ? (
                                        <span className="inline-flex items-center px-2.5 py-0.5 rouneded-full text-xs font-medium bg-emerald-100 text-emerald-800">
                                ...
                                <td className="px-6 py-4 text-right">
                                    <span className="text-sm font-medium text-gray-900"> R$ {produto.preco.toFixed(2)} </span>
                                </td>
                                <td className="px-6 py-4 text-center">
                                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${produto.quantidade > 10 ? 'bg-green-100 text-green-800' : produto.quantidade > 0 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}
                                    >
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex items-center justify-center gap-2">
                                        <button onClick={() => onEditar(produto)} title="Editar Produto" className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                                            <Pencil className="w-4 h-4" />
                                        </button>
                                        <button onClick={() => onDeletar(produto)} title="Deletar Produto" className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors">
